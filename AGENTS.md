# Social Tech Codex Instructions
...
- Prefer search/discovery before opening files.
- Read only the files needed for the task.
- Avoid re-reading unchanged files.
- Avoid copying large file contents into the conversation.
- If project context is missing or outdated, use the `context_manager` agent first.
