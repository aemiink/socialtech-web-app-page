<!-- docs/amazon-ads-phases/08-amazon-ads-sync-automation-hardening.md -->

# FAZ 8 — Amazon Ads Sync Automation, Error Handling ve Rate Limit Güvenliği

## Amaç

Amazon Ads verilerinin düzenli, güvenli ve sürdürülebilir şekilde senkronize edilmesi.
...
4. Error retry
5. Async report request status tracking
...
Amazon Ads API errorları kullanıcı dostu hale getirilmeli:
...
- Amazon Ads API quotas dikkate alınmalı.
- Repeated failed sync kısa sürede tekrar denenmemeli.
- Async report flow için polling limitli olmalı.
...
- teknik Amazon Ads error detayı gösterilmez.
...
1. `AmazonAdsSyncLog` modelini ekle.
2. Sync service logging ekle.
3. Manual sync log yazsın.
4. Reporting v3 async report status tracking için V1-compatible yapı kur.
