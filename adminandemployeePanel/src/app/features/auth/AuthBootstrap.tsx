    ...
    const bootstrap = async () => {
      ...
      try {
        if (accessToken) {
          ...
          if (!isSupportedPanelUser(refreshed.user)) {
            await logout().unwrap().catch(() => undefined);
            dispatch(clearAuth());
            return;
          }
...
function isSupportedPanelUser(user: AuthUserProfile): boolean {
  if (user.accountType === "ADMIN") {
    return user.role === "ADMIN";
  }

  if (user.accountType === "EMPLOYEE") {
    return mapBackendRoleToEmployeeRole(user.role) !== null;
  }
