...
export type GrowthHubReportMutationRequest = {
  projectId?: string | null;
  periodStart?: string;
  periodEnd?: string;
  type?: GrowthHubReportType;
...
export type GrowthHubClientsListItem = {
  client: GrowthHubClientSummary;
  serviceStatus: string | null;
  config: GrowthHubConfig | null;
  state: GrowthHubSummaryState;
  metrics: GrowthHubSummary["metrics"];
  channels: GrowthHubChannelSummary[];
  actions: GrowthHubActionItem[];
  meta: GrowthHubSummary["meta"];
};

export type GrowthHubClientsResponse = {
  data: GrowthHubClientsListItem[];
  meta: {
    total: number;
    ready: number;
    risk: number;
