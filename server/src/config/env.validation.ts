...
export const envValidationSchema = Joi.object({
  ...
  SOCIAL_MEDIA_META_OAUTH_SCOPES: Joi.string().min(1).optional(),
  SOCIAL_MEDIA_META_BUSINESS_LOGIN_CONFIG_ID: Joi.string().min(1).optional(),
  META_WEBHOOK_VERIFY_TOKEN: Joi.string().min(16).optional(),
  ...
  AMAZON_ADS_CLIENT_ID: Joi.string().min(1).optional(),
  AMAZON_ADS_CLIENT_SECRET: Joi.string().min(1).optional(),
  AMAZON_ADS_REDIRECT_URI: Joi.string().uri().optional(),
  AMAZON_ADS_OAUTH_SCOPES: Joi.string().min(1).default("advertising::campaign_management"),
  AMAZON_ADS_DEFAULT_REGION: Joi.string().valid("NA", "EU", "FE").default("NA"),
  AMAZON_ADS_REPORT_POLL_ATTEMPTS: Joi.number().integer().min(1).max(60).default(6),
  AMAZON_ADS_REPORT_POLL_INTERVAL_MS: Joi.number().integer().min(100).max(30000).default(1000),
  AMAZON_ADS_SYNC_TTL_MINUTES: Joi.number().integer().min(1).max(1440).default(30),
  AMAZON_ADS_FAILED_SYNC_RETRY_COOLDOWN_MINUTES: Joi.number().integer().min(1).max(1440).default(10),
