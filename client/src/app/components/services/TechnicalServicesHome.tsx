...
function SharedFooter({ activeServiceIndex }: { activeServiceIndex: number }) {
  return (
...
function HeaderNav({ mobileMenuOpen, setMobileMenuOpen }: { mobileMenuOpen: boolean; setMobileMenuOpen: (value: boolean) => void }) {
  return (
    <header className="relative z-20">
      <div className="mx-auto flex w-full max-w-[1540px] items-center justify-between gap-6 px-6 py-6 lg:px-10">
        ...
        <nav className="hidden items-center gap-10 text-sm lg:flex">
          {navItems.map((item) => (
            <Link key={item.label} className={item.active ? "text-[#b5ff15]" : "text-white/76 transition hover:text-[#b5ff15]"} to={item.to}>
              {item.label}
            </Link>
          ))}
        </nav>
...
function Hero({
  ...
  variant: TechnicalServicesVariant;
...
export default function TechnicalServicesHome({ variant }: { variant: TechnicalServicesVariant }) {
  ...
  return (
    <div className="min-h-screen bg-[#111111] text-white" id="top">
      <Hero mobileMenuOpen={mobileMenuOpen} setMobileMenuOpen={setMobileMenuOpen} variant={variant} />
      {isSeo ? <SeoContent /> : <WebTechnicContent />}
      <PackagesSection variant={variant} />
      {isSeo ? <SeoFaqSection /> : null}
      <SharedFooter activeServiceIndex={isSeo ? 3 : 6} />
    </div>
  );
}
