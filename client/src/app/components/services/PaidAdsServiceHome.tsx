...
const defaultAdFeatures: FeatureData[] = [
...
const googleFeatures: FeatureData[] = [
...
const tiktokFeatures: FeatureData[] = [
...
const mediaHubPackage: PackageData = {
...
const googlePackages: PackageData[] = [
  {
...
const metaPackages: PackageData[] = [
  {
    name: "Marka Bilinirliği Paketi",
...
const tiktokPackages: PackageData[] = [
  {
...
const analyticsIconFallback = reportIcon;

const variants: Record<PaidAdsVariant, VariantData> = {
    ...
    packages: {
      ...
      footer: (
        <>
          ...
          <a className="mx-2 font-bold text-[#aaff01] underline" href="#footer">
            formu
          </a>
...
function TileCard({ title, description, icon, tone }: TileData & { tone: "lime" | "violet" }) {
  const shell =
    tone === "lime"
...
function FeatureTile({ label, icon, tone }: FeatureData & { tone: "lime" | "violet" }) {
...
function ProcessCard({ title, description, icon }: TileData) {
  return (
...
function PaidAdsHeroVisual({ visual }: { visual: HeroVisualData }) {
  return (
          ...
          <div className="mx-auto flex w-full max-w-[1540px] items-center justify-between gap-6 px-6 py-6 lg:px-10">
            ...
            <nav className="hidden items-center gap-10 text-sm lg:flex">
              {navItems.map((item) => (
                <Link
                  key={item.label}
                  className={item.active ? "text-[#aaff01]" : "text-white/76 transition hover:text-[#aaff01]"}
                  to={item.to}
                >
                  {item.label}
                </Link>
