...
- Client Growth Hub feature:
  - `clientPanel/src/app/features/growthHub/growthHubApi.ts` - own-client Growth Hub config/summary/channels/actions/weekly-notes/reports/recommendations/activity RTK Query hooks
  - `clientPanel/src/app/features/growthHub/growthHubTypes.ts` - Growth Hub client dashboard contract types including common channel metric/health/risk, report, and recommendation fields
  - `clientPanel/src/app/features/growthHub/growthHubUtils.ts` - response normalizers, report/recommendation labels, common channel field fallbacks, date/metric formatters, health score helper
...
- Growth Hub client portal Faz 2 touchpoints:
  ...
  - `clientPanel/src/app/pages/services/growth-hub-dashboard.tsx` - API-driven Growth Hub dashboard with loading/error/empty/config states, common channel health/risk cards, service detail targeting, latest visible weekly note rendering, published report cards, and client-visible recommendation cards
  - `clientPanel/src/app/pages/__tests__/growth-hub-dashboard.test.tsx` - summary/channel/action/weekly-note/report/recommendation/activity/error/empty/no-mock regression coverage
- Growth Hub admin/employee Faz 3-4 touchpoints:
  - `adminandemployeePanel/src/app/features/growthHub/growthHubApi.ts` - admin + assigned Growth Hub list/detail/report/recommendation hooks for `/admin/growth-hub/*` ve `/growth-hub/clients/*`
  - `adminandemployeePanel/src/app/features/growthHub/growthHubTypes.ts` / `growthHubUtils.ts` - admin/employee Growth Hub response/report/recommendation types, normalizers, labels ve format helpers
  ...
  - `adminandemployeePanel/src/app/features/growthHub/components/GrowthHubActionNotePanel.tsx` - shared action, weekly note, report and recommendation management panel
...
- `server/prisma/schema.prisma` - Prisma schema foundation:
  ...
  - Growth Hub models: `ClientGrowthHubConfig`, `GrowthHubAction`, `GrowthHubWeeklyNote`, `GrowthHubReport`, `GrowthHubRecommendation` with Growth Hub goal/status/action/report/recommendation enums
...
- `server/src/growth-hub/admin-growth-hub.controller.ts`
  - admin global clients list and admin client-scoped config/summary/channels/actions/reports/recommendations/activity endpoints under `/api/v1/admin/*growth-hub*`
...
- `server/src/growth-hub/growth-hub-management.controller.ts`
  - assigned action/note/report/recommendation patch/delete/publish/convert endpoints under `/api/v1/growth-hub/actions/:actionId`, `/api/v1/growth-hub/weekly-notes/:noteId`, `/api/v1/growth-hub/reports/:reportId`, and `/api/v1/growth-hub/recommendations/:recommendationId`
- `server/src/growth-hub/client-growth-hub.controller.ts`
  - own-client read endpoints under `/api/v1/clients/me/growth-hub/*`, including visible weekly notes, published reports and client-visible recommendations
- `server/src/growth-hub/growth-hub.service.ts`
  - service-level admin/assigned/client permission checks, active `GROWTH_HUB` purchased-service checks, config/action/note/report/recommendation response shaping, report publish -> acknowledgement task bridge, rule-based recommendation generation, recommendation -> task conversion, and endpoint orchestration
...
- `adminandemployeePanel/src/app/features/growthHub/growthHubApi.ts`
  - admin global list + admin detail + assigned list/detail/report/recommendation Growth Hub RTK Query hooks
- `adminandemployeePanel/src/app/features/growthHub/growthHubTypes.ts`
  - Growth Hub admin/employee read model response types, common channel metric/health/risk fields, report/recommendation contracts, enum unions ve client list meta contract'ı
- `adminandemployeePanel/src/app/features/growthHub/growthHubUtils.ts`
  - response/report/recommendation normalizers, label helpers, common channel field fallbacks, metric/date/currency formatters ve status tone helpers
...
- `adminandemployeePanel/src/app/features/growthHub/components/GrowthHubActionNotePanel.tsx`
  - admin/employee tarafından paylaşılan Growth action + weekly note + report + recommendation form/list/visibility/publish/generate/convert controls
...
- `adminandemployeePanel/src/app/pages/GrowthHubAdmin.tsx`
  - admin `/growth-hub` route'u: KPI, müşteri listesi, selected detail, health/risk aware channel cards, channel detail links, config edit, persisted action/note/report/recommendation management
- `adminandemployeePanel/src/app/employee/components/GrowthHubWorkspace.tsx`
  - assigned müşteri listesi, selected summary/channels/actions, health/risk aware channel cards, channel detail links, persisted action/note/report/recommendation management, report acknowledgement, messages ve recent activity bölümleri
...
- `server/prisma/seed.ts` - demo seed foundation:
  ...
  - seeds Growth Hub config for `acme-e-ticaret` and Growth Hub read/manage/action/note/report/recommendation permission mappings
