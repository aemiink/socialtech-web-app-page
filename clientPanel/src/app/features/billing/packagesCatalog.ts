...
function uiKeyToBackendKey(uiKey: string): string {
  return uiKey.replace(/-/g, "_").toUpperCase();
}

export function getCatalogEntryByUiKey(uiKey: string): ServiceCatalogEntry | null {
  return CATALOG_BY_SERVICE_KEY[uiKeyToBackendKey(uiKey)] ?? null;
}

export function getTierByUiKey(uiKey: string, tierKey: string): PackageTier | null {
  return getCatalogEntryByUiKey(uiKey)?.tiers.find((t) => t.key === tierKey) ?? null;
}
