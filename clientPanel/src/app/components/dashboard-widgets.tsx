  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-4">
      {items.map((item) => {
        ...
        return (
          <div key={item.title} className="bg-[#1A1A1A] rounded-2xl p-6 border border-white/[0.08]">
            <div className="flex items-start justify-between mb-4">
              <span className="text-[#A0A0A0] text-sm">{item.title}</span>
            ...
            <div className="text-sm text-[#A0A0A0]">{item.note}</div>
          </div>
        );
      })}
    </div>
  ...
  return (
    <div className="bg-[#1A1A1A] rounded-2xl p-6 border border-white/[0.08]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {items.map((item) => (
          <div key={item.title} className="bg-[#202020] rounded-xl p-4 border border-white/[0.08]">
            ...
            <p className="text-xs text-[#A0A0A0]">{item.note}</p>
          </div>
        ))}
      </div>
    </div>
...
export function AgencyComment({ title, children, author }: { title?: string; children: ReactNode; author: string }) {
  return (
    <div className="bg-gradient-to-br from-[#AAFF01]/5 to-[#7B61FF]/5 rounded-2xl p-6 border border-[#AAFF01]/20">
      <h2 className="text-xl text-white mb-3">{title || 'Ajans Yorumu'}</h2>
      <div className="text-sm text-[#A0A0A0] leading-relaxed space-y-3">{children}</div>
      <div className="flex items-center gap-2 text-sm text-[#A0A0A0] mt-5 pt-4 border-t border-white/[0.08]">
      ...
      <div className="space-y-3">
        {items.map((item) => (
          <div key={item.title} className="bg-[#202020] rounded-xl p-4 border border-white/[0.08]">
            ...
            <p className="text-white text-sm mb-3">{item.title}</p>
        ...
        {items.map((item, index) => (
          <div key={item} className="flex gap-3">
            <div className="flex flex-col items-center">
              ...
              {index < items.length - 1 && <div className="w-px h-10 bg-white/[0.08] mt-2" />}
            </div>
            <div>
              <p className="text-white text-sm">{item}</p>
              <p className="text-xs text-[#A0A0A0] mt-1">{index + 1} saat önce güncellendi</p>
            </div>
          </div>
        ))}
      </div>
