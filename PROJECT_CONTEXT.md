...
`DESIGNER` employees now use an API-driven dashboard summary backed by assigned clients, projects, and tasks; static designer dashboard rows are not used for profile/workload data.
`DESIGNER` employees see a role-shaped `Görevlerim` surface: assigned tasks still come from `/tasks`, but table labels, KPI labels, checklist language, creative/revision badges, approval notes, and channel/reference context are design-oriented instead of Web App delivery-oriented. Supported Social/Meta/TikTok/Amazon creative tasks can create a separate customer approval task from this surface; existing assigned tasks are not mutated into approvals because non-PM employees can only status-update their own tasks.
...
`PROJECT_MANAGER` service workspaces are service-profile aware: Web App uses web-app workspace tabs/endpoints, while Growth Hub, ads, Social Media, design, support, and SEO services use service-specific tabs, task target presets, and reference fields without calling Web App workspace endpoints. The PM task form derives an assignment routing profile from the selected assignee role first, so cross-service assignments such as Growth Hub -> Social Media Specialist open Social Media target tabs, type labels, workflow labels, and target-specific reference fields while keeping the task on the current service project.
...
## Growth Hub Faz 7 — Reports + Acknowledgements (2026-05-30)

Growth Hub rapor lifecycle'ı kanal orkestrasyon katmanına eklendi. Özet:
- Prisma persistence: `GrowthHubReport`, report type/status enum'ları, publish metadata, `clientVisible` kontrolü ve task acknowledgement relation'ı.
- Admin ve assigned employee endpointleri rapor draft/create/update/publish akışını sağlar; own-client endpoint yalnızca `PUBLISHED + clientVisible` raporları döndürür.
- Publish sırasında istenirse mevcut `Task` approval altyapısında `GROWTH_REPORT_ACKNOWLEDGEMENT` acknowledgement task'ı oluşturulur/güncellenir.
- Growth Hub summary/actions ve channel aggregation pending report acknowledgement sinyallerini aynı read model'e dahil eder.
- Admin/employee `GrowthHubActionNotePanel` rapor oluşturma, yayınlama, görünürlük ve acknowledgement aksiyonlarını; client dashboard ise yayınlanmış rapor kartlarını render eder.

Active Growth Hub Faz 7 report endpoints:
- `GET/POST /api/v1/admin/clients/:clientId/growth-hub/reports`
- `PATCH /api/v1/admin/growth-hub/reports/:reportId`
- `POST /api/v1/admin/growth-hub/reports/:reportId/publish`
- `GET/POST /api/v1/growth-hub/clients/:clientId/reports`
- `PATCH /api/v1/growth-hub/reports/:reportId`
